...
# Vemos los distintos valores de municipality y nos quedamos solo con Sevilla
distinct_municipalities_compra = df_compra['municipality'].unique()
df_compra = df_compra[df_compra['municipality'] == 'Sevilla']
df_compra = df_compra[sorted(df_compra.columns)]
...
# BATHROOMS	integer	Dejar como numérica y quitar donde haya columnas nan
df = df[~df['bathrooms'].isin([np.nan, False])]
...
df = df[~(df['floor'].isin(['st', '-1']))] #Quitamos los 'st' y '-1'
...
df.loc[df['newDevelopment'] == 'True', 'newDevelopment'] = True
df.loc[df['newDevelopment'] == 'False', 'newDevelopment'] = False
df.loc[df['newDevelopment'].isna(), 'newDevelopment'] = False
...
# PROPERTYTYPE categórica ['chalet', 'duplex', 'flat', 'penthouse', 'studio'] el resto de valores no han sido entrenados
df = df[df['propertyType'].isin(['chalet', 'duplex', 'flat', 'penthouse', 'studio'])]
...
# ROOMS convertir como numérica. Quitar los 'False' y 'nan'.
df = df[~(df['rooms'].isin(['False']))]
df = df[~(df['rooms'].isna())]
df['rooms'] = pd.to_numeric(df['rooms'], errors='coerce')

# SIZE dejar como numérica
df = df[~(df['size'].isin(['False']))]
df = df[~(df['size'].isna())]
df['size'] = pd.to_numeric(df['size'], errors='coerce')

# STATUS categórica ['good' (0,0), 'newdevelopment' (1,0), 'renew' (0,1)] Eliminar los registros con 'nan'
df = df[~(df['status'].isin(['False']))]
